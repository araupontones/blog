...
During the first few games, our performance was very low (we hit only 33% of the win/lose/draw and have guessed only 1 score. We assumed that the reason was that the FIFA rankings that we were using 1) very static in time and 2) were not very accurate to reflect differences between teams. Thus, we substitute FIFA rankings for ELO ratings to have a better indicator for the team's performance. 

Performance of the first 12 matches

![](images/performance_walle_1.svg)

To have updated data on daily basis, we built a cronjob that ran every day at 1 o'clock in our virtual machine. After every match,this cron-job would re-estimate each team's ELO rating and would create the expectancy of a team wining over its opponent based on their ELO ratings differences. We called this variable *magic_johnson*.  This was based on the [eloratings.net](https://www.eloratings.net/about)
